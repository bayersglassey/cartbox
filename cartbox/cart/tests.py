...
class CartRequestFactory(RequestFactory):
    """Request factory which pokes stuff onto the request so we
    don't need to run middleware"""
...
class CartTestCaseMixin:
    ...
    def setUp(self):
        ...
        # Create default products & categories, poke some onto self
        # so we can place orders with specific items
...
class CartTestCase(CartTestCaseMixin, TestCase):
    ...
    def test_order_items_by_category(self):
        # Place an order, get its items_by_category
...
class CartViewsTestCase(CartTestCaseMixin, TestCase):
    ...
    def setUp(self):
        ...
        # ...I kind of prefer just using self.client instead of calling
        # views by hand. But that's not the approach we took here. Ah well.
    ...
    def test_shop_view(self):
        ...
        # POST to shop view (but don't place order, just show "cart")
        ...
        # Make sure context has a list of suggested products (but didn't
        # find any suggestions, because we didn't set user up with any
        # analytics counters...)
        self.assertLen(context["suggested_products"], 0)

        # POST to shop view, placing an order (by "clicking the place-order
        # submit button", as it were)
        request = self.factory.post('fake_url', {
            'sku': self.banana.sku,
            'sku': self.apple.sku,
            'suggested-sku': self.beef.sku,
        ...
        response = self.shop_view(request)
        # Make sure an order was placed & we were redirected to its page
